...
int list(const char* path, int recursive)
{
	...
	// read all entries in the directory into the names array
	while ((entry = readdir(dir)) != NULL) {
		...
		char *name_path = malloc(strlen(entry->d_name) + strlen(path) + 2); // plus 2 for the '/'
		if (!name_path) {
			closedir(dir);
			return -1;
		}
		...
		if (filetype == S_IFIFO) {
			...
			typestr = buffer;
			strcpy(buffer, " -> ");
			readlinkat(dirfd(dir), entry->d_name, buffer + 4, 1020);
		} 
		else if ((st.st_mode & 0000100)) {
			mode[0] = '-';
			typestr = "*";
		} else {
			mode[0] = '-';
		...
		// ecursively descends into subdirectories if the parameter recursive is not 0. 
